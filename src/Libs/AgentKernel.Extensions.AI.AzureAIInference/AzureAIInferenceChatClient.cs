...
public sealed class AzureAIInferenceChatClient : IChatClient
{
    ...
    private static Microsoft.Extensions.AI.ChatRole ChatRoleDeveloper { get; } = new("developer");
    ...
    private ChatCompletionsOptions ToAzureAIOptions(IEnumerable<ChatMessage> chatContents, ChatOptions? options)
    {
        ...
        {
            Model = options?.ModelId ?? throw new InvalidOperationException("No model id was provided when either constructing the client or in the chat options.")
        };
