...
public static class McpServerExtensions
{
        ...
        this IMcpServer server, CreateMessageRequestParams request, CancellationToken cancellationToken)
    {
        ...
        var p = JsonSerializer.Serialize(request, JsonSerializerOptionsExtensions.JsonContext.Default.CreateMessageRequestParams);
        return server.SendRequestAsync<CreateMessageResult>(
            new JsonRpcRequest { Method = "sampling/createMessage", Params = BinaryData.FromString(p) },
            cancellationToken);
    }
        ...
        this IMcpServer server, ListRootsRequestParams request, CancellationToken cancellationToken)
    {
        ...
        var p = JsonSerializer.Serialize(request, JsonSerializerOptionsExtensions.JsonContext.Default.ListRootsRequestParams);
        return server.SendRequestAsync<ListRootsResult>(
            new JsonRpcRequest { Method = "roots/list", Params = BinaryData.FromString(p) },
            cancellationToken);
    }
