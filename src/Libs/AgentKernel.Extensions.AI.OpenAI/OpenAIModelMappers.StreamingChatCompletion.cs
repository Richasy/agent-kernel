...
{
    public static async IAsyncEnumerable<Core.OpenAI.Chat.StreamingChatCompletionUpdate> ToOpenAIStreamingChatCompletionAsync(
        IAsyncEnumerable<Microsoft.Extensions.AI.StreamingChatCompletionUpdate> chatCompletions,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        await foreach (var chatCompletionUpdate in chatCompletions.WithCancellation(cancellationToken).ConfigureAwait(false))
        {
            ...
            yield return OpenAIChatModelFactory.StreamingChatCompletionUpdate(
                ...
                refusalUpdate: chatCompletionUpdate.AdditionalProperties.GetValueOrDefault<string>(nameof(Core.OpenAI.Chat.StreamingChatCompletionUpdate.RefusalUpdate)),
                contentTokenLogProbabilities: chatCompletionUpdate.AdditionalProperties.GetValueOrDefault<IReadOnlyList<ChatTokenLogProbabilityDetails>>(nameof(Core.OpenAI.Chat.StreamingChatCompletionUpdate.ContentTokenLogProbabilities)),
                refusalTokenLogProbabilities: chatCompletionUpdate.AdditionalProperties.GetValueOrDefault<IReadOnlyList<ChatTokenLogProbabilityDetails>>(nameof(Core.OpenAI.Chat.StreamingChatCompletionUpdate.RefusalTokenLogProbabilities)),
                systemFingerprint: chatCompletionUpdate.AdditionalProperties.GetValueOrDefault<string>(nameof(Core.OpenAI.Chat.StreamingChatCompletionUpdate.SystemFingerprint)),
                usage: chatTokenUsage);
        }
    }

    public static async IAsyncEnumerable<Microsoft.Extensions.AI.StreamingChatCompletionUpdate> FromOpenAIStreamingChatCompletionAsync(
        IAsyncEnumerable<Core.OpenAI.Chat.StreamingChatCompletionUpdate> chatCompletionUpdates,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (Core.OpenAI.Chat.StreamingChatCompletionUpdate chatCompletionUpdate in chatCompletionUpdates.WithCancellation(cancellationToken).ConfigureAwait(false))
        {
            ...
            finishReason ??= chatCompletionUpdate.FinishReason is Core.OpenAI.Chat.ChatFinishReason reason ? FromOpenAIFinishReason(reason) : null;
            completionId ??= chatCompletionUpdate.CompletionId;
            ...
            Microsoft.Extensions.AI.StreamingChatCompletionUpdate completionUpdate = new()
            {
                CompletionId = chatCompletionUpdate.CompletionId,
                CreatedAt = chatCompletionUpdate.CreatedAt,
        ...
        if (functionCallInfos is not null)
        {
            ...
            if (fingerprint is not null)
            {
                (completionUpdate.AdditionalProperties ??= [])[nameof(Core.OpenAI.Chat.ChatCompletion.SystemFingerprint)] = fingerprint;
            }
