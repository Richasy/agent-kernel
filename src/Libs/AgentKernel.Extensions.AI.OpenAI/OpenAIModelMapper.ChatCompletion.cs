...
internal static partial class OpenAIModelMappers
{
    ...
    public static Core.OpenAI.Chat.ChatCompletion ToOpenAIChatCompletion(Microsoft.Extensions.AI.ChatCompletion chatCompletion, JsonSerializerOptions options)
    {
        ...
        List<Core.OpenAI.Chat.ChatToolCall>? toolCalls = null;
        ...
        Core.OpenAI.Chat.ChatTokenUsage? chatTokenUsage = null;
        ...
        return OpenAIChatModelFactory.ChatCompletion(
            ...
            toolCalls: toolCalls,
            refusal: chatCompletion.AdditionalProperties.GetValueOrDefault<string>(nameof(Core.OpenAI.Chat.ChatCompletion.Refusal)),
            contentTokenLogProbabilities: chatCompletion.AdditionalProperties.GetValueOrDefault<IReadOnlyList<ChatTokenLogProbabilityDetails>>(nameof(Core.OpenAI.Chat.ChatCompletion.ContentTokenLogProbabilities)),
            refusalTokenLogProbabilities: chatCompletion.AdditionalProperties.GetValueOrDefault<IReadOnlyList<ChatTokenLogProbabilityDetails>>(nameof(Core.OpenAI.Chat.ChatCompletion.RefusalTokenLogProbabilities)),
            systemFingerprint: chatCompletion.AdditionalProperties.GetValueOrDefault<string>(nameof(Core.OpenAI.Chat.ChatCompletion.SystemFingerprint)),
            usage: chatTokenUsage);
    }

    public static Microsoft.Extensions.AI.ChatCompletion FromOpenAIChatCompletion(Core.OpenAI.Chat.ChatCompletion openAICompletion, ChatOptions? options)
    {
        _ = Throw.IfNull(openAICompletion);

        // Create the return message.
        Microsoft.Extensions.AI.ChatMessage returnMessage = new()
        {
        ...
        var completion = new Microsoft.Extensions.AI.ChatCompletion([returnMessage])
        {
            RawRepresentation = openAICompletion,
    ...
    public static ChatOptions FromOpenAIOptions(Core.OpenAI.Chat.ChatCompletionOptions? options)
    {
...
#pragma warning restore OPENAI001
    ...
    private static Microsoft.Extensions.AI.ChatFinishReason? FromOpenAIFinishReason(Core.OpenAI.Chat.ChatFinishReason? finishReason) =>
        finishReason?.ToString() is not string s ? null :
        finishReason switch
        {
            Core.OpenAI.Chat.ChatFinishReason.Stop => Microsoft.Extensions.AI.ChatFinishReason.Stop,
            Core.OpenAI.Chat.ChatFinishReason.Length => Microsoft.Extensions.AI.ChatFinishReason.Length,
            Core.OpenAI.Chat.ChatFinishReason.ContentFilter => Microsoft.Extensions.AI.ChatFinishReason.ContentFilter,
            Core.OpenAI.Chat.ChatFinishReason.ToolCalls or Core.OpenAI.Chat.ChatFinishReason.FunctionCall => Microsoft.Extensions.AI.ChatFinishReason.ToolCalls,
            _ => new Microsoft.Extensions.AI.ChatFinishReason(s),
        };

    /// <summary>Converts an Extensions finish reason to an OpenAI finish reason.</summary>
    private static Core.OpenAI.Chat.ChatFinishReason ToOpenAIFinishReason(Microsoft.Extensions.AI.ChatFinishReason? finishReason) =>
        finishReason == Microsoft.Extensions.AI.ChatFinishReason.Length ? Core.OpenAI.Chat.ChatFinishReason.Length :
        finishReason == Microsoft.Extensions.AI.ChatFinishReason.ContentFilter ? Core.OpenAI.Chat.ChatFinishReason.ContentFilter :
        finishReason == Microsoft.Extensions.AI.ChatFinishReason.ToolCalls ? Core.OpenAI.Chat.ChatFinishReason.ToolCalls :
        Core.OpenAI.Chat.ChatFinishReason.Stop;
